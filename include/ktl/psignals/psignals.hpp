...
namespace psignals {
    ...
    class sigset;
    namespace this_thread { namespace impl { sigset get_mask(); } }
    ...
    */
    ...
    class sigset
    {
     public:
       ...
       ~sigset() noexcept = default;
        ...
        native_handle_type native_handle() const { return const_cast<native_handle_type>(&m_handle); }

        typedef std::unordered_set<signum_t> signals_type;
        const signals_type& signals() const { return m_signals; }

        void insert(const signum_t signum) noexcept { ::sigaddset(&m_handle, signum); m_signals.insert(signum); }
        void erase(const signum_t signum) noexcept { ::sigdelset(&m_handle, signum); m_signals.erase(signum); }

        sigset& operator+=(const signum_t signum) noexcept { insert(signum); return *this; }
        sigset& operator-=(const signum_t signum) noexcept { erase(signum); return *this; }
        ...
        void clear() { ::sigemptyset(&m_handle); m_signals.clear(); }

     private:
        friend sigset this_thread::impl::get_mask(); // for init_handle()

        void copy_handle(const sigset& that) { std::memcpy(&m_handle, &that.m_handle, sizeof(m_handle)); }
        void init_handle(std::initializer_list<signum_t> signums) { clear(); for (signum_t signum : signums) { insert(signum); } }
        void init_handle()
        {
            for (signum_t signum : std::vector<signum_t>{SIGTERM, SIGINT, SIGHUP, SIGUSR1, SIGUSR2, SIGPWR, SIGALRM})
    ...
    inline signum_t wait(const sigset& signals, std::chrono::nanoseconds timeout, ::siginfo* info = nullptr)
    {
        ::timespec ts;
